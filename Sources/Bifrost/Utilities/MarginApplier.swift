...
class MarginApplier {
    ...
    func tryApplyMargin(properties: [DynamicProperty], to view: UIView, in container: UIView) {
        let gravity = properties.first(where: { $0.name == "margin" })?.value as? Margin ?? Margin(left: 0, right: 0, top: 0, bottom: 0)
